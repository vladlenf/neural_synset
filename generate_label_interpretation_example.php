...
else
{
    ...
    $query = "SELECT page_title, label.short_name, wiki_text.text AS wiki_text, quote.text FROM quote, page,lang_pos,wiki_text,label, label_meaning, meaning WHERE quote.meaning_id=meaning.id and page.id=lang_pos.page_id and lang_pos.lang_id=809 and lang_pos.id=meaning.lang_pos_id and meaning.wiki_text_id=wiki_text.id and label_meaning.label_id=label.id and label_meaning.meaning_id=meaning.id and wiki_text.id=meaning.wiki_text_id order by page.page_title;";
    $result = $mysqli->query($query);
    if ($result->num_rows > 0) {
        // Открываем файл для записи
        $fp = fopen('../data/label_interpretation_example.csv', 'w');
